...
public class TDate extends java.util.Date {
     ...
     */
    @Deprecated
    public TDate(int theYear, int theMonth, int theDay) {
    ...
    */ 
     ...
     *             format.
     */
    public static TDate valueOf(String dateString) {
        if (dateString == null) {
            throw new IllegalArgumentException();
        }
        ...
        int day = Integer.parseInt(dateString.substring(secondIndex + 1,
                dateString.length()));
        return new TDate(year - 1900, month - 1, day);
    }
