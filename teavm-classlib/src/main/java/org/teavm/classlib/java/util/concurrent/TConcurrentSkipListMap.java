...
/**
 * A scalable concurrent {@link TConcurrentNavigableMap} implementation.
 ...
 * @since 1.6
 */
public class TConcurrentSkipListMap<K, V> extends AbstractMap<K, V>
    implements TConcurrentNavigableMap<K, V>,
               Cloneable,
    ...
    private transient TConcurrentNavigableMap<K, V> descendingMap;
        ...
        for (;;) {
            ...
            if (r != null && (n = r.node) != bound && (k = n.key) != null) {
                if ((c = key.compareTo(k)) > 0) {
                    ...
                    Object v = n.value;
                    return (V) ((v != null)? v : getUsingFindNode(key));
                } else
                    bound = n;
            }
     ...
     */
    public TConcurrentSkipListMap(Comparator comparator) {
     ...
     *         or values are null
     */
    public TConcurrentSkipListMap(Map m) {
        this.comparator = null;
        initialize();
        putAll(m);
    }
     ...
     */
    public TConcurrentSkipListMap(SortedMap m) {
     ...
     */
    public Object clone() {
        TConcurrentSkipListMap clone = null;
        try {
            clone = (TConcurrentSkipListMap) super.clone();
    ...
    public NavigableMap<K, V> descendingMap() {
        TConcurrentNavigableMap dm = descendingMap;
    ...
    abstract class Iter implements Iterator {
        ...
        public void remove() {
            ...
            TConcurrentSkipListMap.this.remove(l.key);
            lastReturned = null;
        }
    ...
    static final class KeySet extends AbstractSet implements NavigableSet {
        private final TConcurrentNavigableMap m;
        KeySet(TConcurrentNavigableMap map) { m = map; }
        ...
        public Iterator iterator() {
            if (m instanceof TConcurrentSkipListMap)
                return ((TConcurrentSkipListMap)m).keyIterator();
            else
                return ((TConcurrentSkipListMap.SubMap)m).keyIterator();
        }
        public boolean equals(Object o) {
            if (o == this)
                return true;
        ...
        public NavigableSet subSet(Object fromElement,
                                      ...
                                      Object toElement,
                                      boolean toInclusive) {
            return new TConcurrentSkipListSet
                ((TConcurrentNavigableMap)
                 m.subMap(fromElement, fromInclusive,
                          toElement,   toInclusive));
        }
        public NavigableSet headSet(Object toElement, boolean inclusive) {
            return new TConcurrentSkipListSet(
                (TConcurrentNavigableMap)m.headMap(toElement, inclusive));
        }
        public NavigableSet tailSet(Object fromElement, boolean inclusive) {
            return new TConcurrentSkipListSet(
                (TConcurrentNavigableMap)m.tailMap(fromElement, inclusive));
        }
        ...
        public NavigableSet descendingSet() {
            return new TConcurrentSkipListSet(
                (TConcurrentNavigableMap)m.descendingMap());
        }
    }

    static final class Values extends AbstractCollection {
        private final TConcurrentNavigableMap m;
        Values(TConcurrentNavigableMap map) {
            m = map;
        }
        public Iterator iterator() {
            if (m instanceof TConcurrentSkipListMap)
                return ((TConcurrentSkipListMap)m).valueIterator();
            else
    ...
    static final class EntrySet extends AbstractSet {
        private final TConcurrentNavigableMap m;
        EntrySet(TConcurrentNavigableMap map) {
            m = map;
        }

        public Iterator iterator() {
            if (m instanceof TConcurrentSkipListMap)
                return ((TConcurrentSkipListMap)m).entryIterator();
            else
    ...
    /**
     * Submaps returned by {@link TConcurrentSkipListMap} submap operations
    ...
    static final class SubMap extends AbstractMap
        implements TConcurrentNavigableMap, Cloneable,
                   java.io.Serializable {
        private static final long serialVersionUID = -7647078645895051609L;

        /** Underlying map */
        private final TConcurrentSkipListMap m;
         ...
         */
        SubMap(TConcurrentSkipListMap map,
         ...
         */
        private boolean isBeforeEnd(TConcurrentSkipListMap.Node n) {
            if (n == null)
                return false;
            if (hi == null)
                return true;
         ...
         */
        private TConcurrentSkipListMap.Node loNode() {
            if (lo == null)
                return m.findFirst();
            else if (loInclusive)
         ...
         * most usages need to check bounds
         */
        private TConcurrentSkipListMap.Node hiNode() {
            if (hi == null)
                return m.findLast();
            else if (hiInclusive)
        ...
        private Object lowestKey() {
            TConcurrentSkipListMap.Node n = loNode();
            if (isBeforeEnd(n))
                return n.key;
            else
                throw new NoSuchElementException();
        ...
        private Object highestKey() {
            TConcurrentSkipListMap.Node n = hiNode();
            if (n != null) {
                Object last = n.key;
                if (inBounds(last))
                    return last;
        ...
        public int size() {
            long count = 0;
            for (TConcurrentSkipListMap.Node n = loNode();
                 isBeforeEnd(n);
                 n = n.next) {
                if (n.getValidValue() != null)
                    ++count;
