 ...
 * @see StringReader
 */
public class TStringWriter extends TWriter {
	 ...
	 */
	@Override
	public TStringWriter append(TCharSequence csq) {
		if (null == csq) {
			write(TOKEN_NULL);
		} else {
			write(csq.toString());
	 ...
	 */
	@Override
	public TStringWriter append(TCharSequence csq, int start, int end) {
		final String output;
		if (null == csq) {
			output = TOKEN_NULL.subSequence(start, end).toString();
		} else {
