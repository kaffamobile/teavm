 ...
 * </pre>
 *
 * <p>In addition to implementing the {@link TLock} interface, this
...
public class TReentrantLock implements TLock, java.io.Serializable,
    ...
    static abstract class Sync implements java.io.Serializable {
        ...
        /**
         * Performs {@link TLock#lock}. The main reason for subclassing
     ...
     * {@link #tryLock(long, TTimeUnit) tryLock(0, TimeUnit.SECONDS) }
     ...
     * <ul>
     *
     * <li>If this lock is not held when any of the {@link TCondition}
     * {@linkplain TCondition#await() waiting} or {@linkplain
     * TCondition#signal signalling} methods are called, then an {@link
    ...
    public TCondition newCondition() {
        return isFair() ? (TCondition)new TFIFOCondVar(this) : new TCondVar(this);
    }
     ...
     * @throws NullPointerException if the condition is null
     */
    protected Collection getWaitingThreads(TCondition condition) {
    ...
    private TCondVar asCondVar(TCondition condition) {
        if (condition == null)
            throw new NullPointerException();
        if (!(condition instanceof TCondVar))
            throw new IllegalArgumentException("not owner");
        TCondVar condVar = (TCondVar)condition;
