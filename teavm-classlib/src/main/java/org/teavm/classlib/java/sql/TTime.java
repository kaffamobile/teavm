 ...
 */
public class TTime extends Date {
     ...
     */
    @Deprecated
    public TTime(int theHour, int theMinute, int theSecond) {
    ...
    */ 
     ...
     */
    public static TTime valueOf(String timeString) {
        if (timeString == null) {
            throw new IllegalArgumentException();
        }
        ...
        int second = Integer.parseInt(timeString.substring(secondIndex + 1,
                timeString.length()));
        return new TTime(hour, minute, second);
    }
}
