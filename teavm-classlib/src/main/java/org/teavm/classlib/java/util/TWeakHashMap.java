...
/**
 * WeakHashMap is an implementation of TMap with keys which are WeakReferences. A
...
public class TWeakHashMap<K, V> extends TAbstractMap<K, V> implements TMap<K, V> {
    ...
    TCollection<V> valuesCollection;
    ...
    private static final class Entry<K, V> extends WeakReference<K> implements
        ...
        public boolean equals(Object other) {
            if (!(other instanceof TMap.Entry)) {
                return false;
            }
            TMap.Entry<?, ?> entry = (TMap.Entry<?, ?>) other;
     ...
     * an instance of {@link TMap.Entry}. As the set is backed by this map,
    ...
    public TSet<TMap.Entry<K, V>> entrySet() {
        ...
        return new TAbstractSet<TMap.Entry<K, V>>() {
            ...
            public boolean remove(Object object) {
                if (contains(object)) {
                    TWeakHashMap.this
                            .remove(((TMap.Entry<?, ?>) object).getKey());
                    return true;
                }
                return false;
            }
            ...
            public boolean contains(Object object) {
                if (object instanceof TMap.Entry) {
                    Entry<?, ?> entry = getEntry(((TMap.Entry<?, ?>) object)
                            .getKey());
                    if (entry != null) {
            ...
            public TIterator<TMap.Entry<K, V>> iterator() {
                return new HashIterator<TMap.Entry<K, V>>(
                        new Entry.Type<TMap.Entry<K, V>, K, V>() {
                            public TMap.Entry<K, V> get(TMap.Entry<K, V> entry) {
                                return entry;
                            }
                        });
            }
            ...
            keySet = new TAbstractSet<K>() {
                ...
                public TIterator<K> iterator() {
                    return new HashIterator<K>(new Entry.Type<K, K, V>() {
                        public K get(TMap.Entry<K, V> entry) {
                            return entry.getKey();
                        }
                    });
                }
                ...
                public Object[] toArray() {
                    ...
                    for (TIterator<K> iter = iterator(); iter.hasNext();) {
                        coll.add(iter.next());
                    }
                    return coll.toArray();
                }
    ...
    public TCollection<V> values() {
        ...
        if (valuesCollection == null) {
            valuesCollection = new TAbstractCollection<V>() {
                @Override
                public int size() {
                    return TWeakHashMap.this.size();
                }
                ...
                public TIterator<V> iterator() {
                    return new HashIterator<V>(new Entry.Type<V, K, V>() {
                        public V get(TMap.Entry<K, V> entry) {
