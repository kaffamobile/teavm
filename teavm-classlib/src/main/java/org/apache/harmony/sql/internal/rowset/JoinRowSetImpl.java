...
public class JoinRowSetImpl extends WebRowSetImpl implements JoinRowSet {
    ...
    private void composeMetaData(TResultSetMetaData rsmd, int matchColumn)
            throws TSQLException {
        if (getMetaData() == null) {
    ...
    public void addRowSet(Joinable rowset) throws TSQLException {
        if (rowset == null || !(rowset instanceof RowSet)) {
            // rowset.33=Not a rowset
            throw new TSQLException(Messages.getString("rowset.33")); //$NON-NLS-1$
        }

        RowSet currentRs = (RowSet) rowset;
        if (currentRs.getMetaData() == null) {
            // rowset.32=The given rowset is empty
            throw new TSQLException(Messages.getString("rowset.32")); //$NON-NLS-1$
        }
        ...
        try {
            ...
            try {
                if (rowset.getMatchColumnNames() != null
                    ...
                    } catch (TSQLException e1) {
                        matchCol = -3;
                    }
                }
            } catch (TSQLException e2) {
                // ignore
            }
        } finally {
            if (matchCol == -1) {
                // rowset.34=Not set a match column
                throw new TSQLException(Messages.getString("rowset.34")); //$NON-NLS-1$
            } else if (matchCol == -2) {
                // rowset.35=Not a valid match olumn index
                throw new TSQLException(Messages.getString("rowset.35")); //$NON-NLS-1$
            } else if (matchCol == -3) {
    ...
    public void addRowSet(RowSet rowset, int columnIdx) throws TSQLException {
        if (rowset == null) {
        ...
        if (getMetaData() == null
                && (type == TTypes.BINARY || type == TTypes.LONGVARBINARY
                        || type == TTypes.VARBINARY || type == TTypes.BLOB
                        || type == TTypes.CLOB || type == TTypes.ARRAY
                        || type == TTypes.LONGVARCHAR
                        || type == TTypes.JAVA_OBJECT || type == TTypes.OTHER || type == TTypes.NULL)) {
            // rowset.10=Data Type Mismatch
            throw new TSQLException(Messages.getString("rowset.10")); //$NON-NLS-1$
        }
        ...
        try {
            ...
            TSQLException sqlException = new TSQLException();
    ...
    public void addRowSet(RowSet rowset, String columnName) throws TSQLException {
        if (rowset == null) {
            throw new NullPointerException();
        }
        if (rowset.getMetaData() == null) {
    ...
    public void addRowSet(RowSet[] rowset, int[] columnIdx) throws TSQLException {
        ...
        if (rowset.length != columnIdx.length) {
            ...
            throw new TSQLException(Messages.getString("rowset.36")); //$NON-NLS-1$
        }
    ...
    public int getJoinType() throws TSQLException {
        return joinType;
    }

    public String[] getRowSetNames() throws TSQLException {
        ...
        for (int i = 0; i < rsList.size(); i++) {
            if (rsList.get(i) instanceof CachedRowSet) {
                ...
                if (cachedRs.getTableName() == null) {
                    ...
                    throw new TSQLException(Messages.getString("rowset.37")); //$NON-NLS-1$
                }
    ...
    public Collection<?> getRowSets() throws TSQLException {
        return rsList;
    }
     ...
     */
    public String getWhereClause() throws TSQLException {
        ...
        if (size == 1) {
            ...
            if (tableName == null) {
                throw new TSQLException(Messages.getString("rowset.39")); //$NON-NLS-1$
            }
    ...
    public void setJoinType(int joinType) throws TSQLException {
        if (supportsJoinType(joinType)) {
            this.joinType = joinType;
        } else {
            // rowset.38=This type of join is not supported
            throw new TSQLException(Messages.getString("rowset.38")); //$NON-NLS-1$
        }
    }
    ...
    public CachedRowSet toCachedRowSet() throws TSQLException {
        if (rsList.size() == 0) {
     ...
     */
    private void innerJoin() throws TSQLException, CloneNotSupportedException {
    ...
    private void iterativeJoinRows(CachedRowSetImpl rowSetToAdd,
            ...
            CachedRowComparator comparator) throws TSQLException {
        // The row from itself.
        CachedRow row;
