...
public class WebRowSetImpl extends CachedRowSetImpl implements WebRowSet {
    ...
    public void readXml(Reader reader) throws TSQLException {
        new XmlReaderImpl().readXML(this, reader);
    }

    public void readXml(InputStream iStream) throws TSQLException, IOException {
        new XmlReaderImpl().readXML(this, new InputStreamReader(iStream));
    }

    public void writeXml(TResultSet rs, Writer writer) throws TSQLException {
        super.populate(rs);
        writeXml(writer);
        beforeFirst();
    }

    public void writeXml(TResultSet rs, OutputStream oStream)
            throws TSQLException, IOException {
        super.populate(rs);
        writeXml(oStream);
        beforeFirst();
    }

    public void writeXml(Writer writer) throws TSQLException {
        new XmlWriterImpl().writeXML(this, writer);
    }

    public void writeXml(OutputStream oStream) throws TSQLException, IOException {
