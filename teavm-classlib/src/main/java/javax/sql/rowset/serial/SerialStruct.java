 ...
 */
public class SerialStruct implements TStruct, Serializable, Cloneable {
     ...
     */
    public SerialStruct(TSQLData in, Map<String, Class<?>> map)
            throws SerialException {
        try {
        ...
        Vector v = new Vector();
        try {
            TSQLOutput out = new SQLOutputImpl(v, map);
            in.writeSQL(out);
        } catch (TSQLException e) {
            throw new SerialException(e.getMessage());
        }
     ...
     *             if there is something wrong.
     */
    public SerialStruct(TStruct in, Map<String, Class<?>> map)
            throws SerialException {
        try {
            SQLTypeName = in.getSQLTypeName();
