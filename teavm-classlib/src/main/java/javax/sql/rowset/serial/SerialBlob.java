...
public class SerialBlob implements TBlob, Serializable, Cloneable {
     ...
     */
    public SerialBlob(TBlob blob) throws SerialException, TSQLException {
        if (blob == null) {
            throw new TSQLException(Messages.getString("sql.13")); //$NON-NLS-1$
        }
        this.blob = blob;
     ...
     */
    public SerialBlob(byte[] buf) throws SerialException, TSQLException {
     ...
     *             if an error is encountered
     */
    public long position(TBlob pattern, long start) throws SerialException,
            TSQLException {
